...
- **Emulator:** Kitchen Display (`listQueueForBranch`/`advanceStage` against real completed/voided orders), Barcode (`getItemByBarcode` against a real seeded item, a real concurrent-`quickSale`-with-the-same-draftId idempotency proof), WhatsApp (`connectWhatsAppChannel`→`syncWhatsAppNotifications` against a real seeded notification, proving one message sent and no re-send on a **normal** repeat sync call — with `platform/secrets` and the WhatsApp Cloud API's own `fetch` calls faked, the same "fake only the one non-Firestore boundary" precedent Phase 5 established), AI Assistant (`answerQuestion` grounded in real seeded stock/audit data, with only the LLM call faked). A separate unit test (`whatsapp-channel.service.test.ts`) proves the narrower, honest crash-recovery bound described in §5 — resuming from a cursor that already advanced past one message never resends that specific message — which is not the same claim as "duplicate delivery is impossible": a crash at the exact instant between a message's own send and its cursor write is not provably ruled out, only bounded to that single message. See §5.
