    ...
    <div className="flex flex-col gap-1 rounded-md border border-neutral-200 p-3">
      ...
      <div className="flex items-center gap-2">
        <form action={quantityAction} className="flex items-center gap-2">
          ...
          <Input type="number" name="quantity" min={1} defaultValue={line.quantity} className="w-20" />
...
export function SalePanel({
  ...
  const [completeState, completeAction, completePending] = useActionState(completeSaleAction, initialState);
  const [voidState, voidAction, voidPending] = useActionState(voidSaleAction, initialState);

  const isPending = order.status === "pending";

  return (
    <main className="mx-auto flex min-h-screen max-w-2xl flex-col gap-6 p-8">
      <h1 className="text-lg font-semibold text-neutral-900">Sale {order.id}</h1>
        ...
        <div className="flex gap-3">
          <form action={completeAction}>
            ...
            <Button type="submit" disabled={completePending}>
              {completePending ? "Completing…" : "Complete sale"}
            </Button>
          </form>
